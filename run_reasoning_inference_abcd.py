...
Usage:
python run_reasoning_inference_abcd.py \
...
def build_prompt_lora_only(question: str) -> str:
    return f"""{SYSTEM_PROMPT}

Используй только корпоративные архитектурные паттерны и best practices.
Если контекст ландшафта не дан, не придумывай конкретные системы, а дай наиболее подходящую архитектурную рекомендацию в стиле нашей компании.
...
def main():
    ...
    rag_context = load_text(args.rag_context_file) if args.rag_context_file else ""
    ...
    variants = [
        ...
        ("D. Reasoning LoRA only", model_lora, tokenizer_lora, build_prompt_lora_only(question)),
    ]

    if rag_context:
        variants.insert(1, ("B. Base model + RAG", model_base, tokenizer_base, build_prompt_with_rag(question, rag_context)))
        variants.insert(3, ("C. Reasoning LoRA + RAG", model_lora, tokenizer_lora, build_prompt_with_rag(question, rag_context)))
    ...
    md.append("- D = answer from reasoning-tuned company-style model without RAG")
    md.append("")
